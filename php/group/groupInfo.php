...
try{
  ...
  $sql = "select GROUP_NO,GROUP_STATUS,GROUP_NAME,GROUP_PIC1,GROUP_PIC2,GROUP_PIC3, CAM_NO ,CAM_AREA, CAM_COUNTY,CAM_NAME, GROUP_START_DATE ,GROUP_MEMNO,MEM_IMG,MEM_NICKNAME,GROUP_INTRO,date(GROUP_DEPART_DATE) `GROUP_DEPART_DATE`,date(GROUP_DEADLINE) `GROUP_DEADLINE`,GROUP_PEOPLE_LIMIT,(GROUP_PEOPLE_LIMIT-GROUP_PEOPLE_SIGNUP) `REMAIN_PEOPLE` FROM campinggroups a JOIN member b ON a.GROUP_MEMNO = b.MEMNO JOIN CAMPING c on c.CAM_NO = a.GROUP_CAM_NO where GROUP_NO = $GROUP_NO;";
  $groupRows = $pdo->query($sql);
  $groupRow[0] = $groupRows->fetch(PDO::FETCH_ASSOC);
  $grouparea=$groupRow[0]["CAM_AREA"];
  // echo $grouparea;
  //留言
  // $sql = "select GROUP_MES_NO, GROUP_MES_MEMNO, MEM_NICKNAME, MEM_IMG, GROUP_MES_TIME , GROUP_MES_CONTENT, GROUP_MES_STATUS from GROUP_MES a JOIN member b on a.GROUP_MES_MEMNO = b.MEMNO where GROUP_MES_STATUS !=1 AND GROUP_MES_GROUPNO = $GROUP_NO;";
  $sql = "select GROUP_MES_NO, GROUP_MES_MEMNO, MEM_NICKNAME, MEM_IMG, GROUP_MES_TIME , GROUP_MES_CONTENT, GROUP_MES_STATUS , REGROUP_MES_NO, 	REGROUP_DEAL
  from GROUP_MES a JOIN member b on a.GROUP_MES_MEMNO = b.MEMNO left join REPORTGROUP_MES c on c.REGROUP_MES_GROUP_NO = a.GROUP_MES_NO
  where GROUP_MES_STATUS !=1 AND GROUP_MES_GROUPNO = $GROUP_NO
  group by GROUP_MES_NO;";
