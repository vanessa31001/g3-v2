...
try{
  ...
  $sql = "select GROUP_NO,GROUP_NAME,GROUP_PIC1,GROUP_PIC2,GROUP_PIC3, CAM_NO ,CAM_AREA, CAM_COUNTY,CAM_NAME, GROUP_START_DATE ,GROUP_MEMNO,MEM_IMG,MEM_NICKNAME,GROUP_INTRO,date(GROUP_DEPART_DATE) `GROUP_DEPART_DATE`,date(GROUP_DEADLINE) `GROUP_DEADLINE`,GROUP_PEOPLE_LIMIT,(GROUP_PEOPLE_LIMIT-GROUP_PEOPLE_SIGNUP) `REMAIN_PEOPLE` FROM campinggroups a JOIN member b ON a.GROUP_MEMNO = b.MEMNO JOIN CAMPING c on c.CAM_NO = a.GROUP_CAM_NO where GROUP_NO = $GROUP_NO;";
  $groupRows = $pdo->query($sql);
